...
let logger: ILogger = new ConsoleLogger("decorator sample");

/**
 * how to define a method decorator
 */
function log(
...
class Actor {
	...
	saySomething(subject: string): string {
		console.log("[" + this.name + "] I'm talking about '" + subject);

		return "Let's hope to do a good job!";
	}
}

let a = new Actor("Alessandro");
let moreWords = a.saySomething("TypeScript");
